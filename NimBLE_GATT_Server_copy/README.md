...
# NimBLE GATT Server Example with INMP441 Audio Support
...
2. How to handle characteristic access requests
    ...
    3. **NEW**: Audio streaming from INMP441 digital microphone via BLE
3. **NEW**: I2S audio recording and real-time BLE transmission

### New Audio Features

- **INMP441 Digital Microphone Support**: High-quality I2S digital microphone integration
- **Real-time Audio Streaming**: 16kHz, 16-bit mono audio over BLE
- **Audio GATT Service**: Custom BLE service for audio data transmission
- **Python Audio Client**: Complete test client for receiving and saving audio data
...
**Note**: This example is specifically designed for ESP32-S3 due to I2S requirements.

### Hardware Setup

1. Connect INMP441 digital microphone to ESP32-S3:
   - See `INMP441_HARDWARE_GUIDE.md` for detailed connection guide
...
> **📝 Flash Audio Files**: 项目已配置自动生成Flash分区镜像，包含23个WAV音频文件。
> 编译时会自动创建 `build/flash.bin` (4MB)，烧录时自动写入ESP32。
> 详细说明：[flash_image/README.md](flash_image/README.md)
...
2. Run the audio test client:
``` shell
python audio_client.py 10
```

This will:
- Scan for ESP32-S3 BLE device
...
```
ESP32 INMP441 Audio Client
Scanning for BLE devices...
Found potential device: NimBLE_GATT (XX:XX:XX:XX:XX:XX)
Connecting to NimBLE_GATT...
Connected to NimBLE_GATT
Started audio notifications for 10 seconds...
Received 200 bytes, max sample: 1234
...
Recording complete. Received 32000 bytes
Audio saved to esp32_audio_20241104_123456.wav
Duration: 10.00 seconds
Max amplitude: 15678
Average amplitude: 234.5
```
